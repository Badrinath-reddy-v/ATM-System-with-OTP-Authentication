...
client = Client("AC4ad077c174f173d7e9034328852016a8",
                "ba915e62ac3e380257074db65e76280a")
...
def Submit():
    ...
    def Withdraw():
        ...
        def Check1():
            if T3.get() == "":
                ...
                messagebox.showinfo(
                    "Error!", "Withdraw Amount Exceeds Allowance")
                T3.delete('0', END)
                T3.focus_force()
            else:
                bal = list(balance)
                ...
                c.execute(
                    "UPDATE Database SET balance=(?) WHERE accno=(?) ", (bal[0], accno))
                conn.commit()
                messagebox.showinfo(
                    "Succesfull!", "Rs"+T3.get()+" is Withdrawn\nNew Balance: Rs"+str(bal[0]))
                T3.delete('0', END)
                L4['text'] = bal[0]
                r1.destroy()
            conn.commit()
        ...
        r1.title("Withdraw")
        L1 = Label(r1, text="Withdraw Menu", bg="darkblue",
                   fg="orange", font=(None, 25), padx=50)
        L1.place(x=60, y=50)
        L2 = Label(r1, text="Current Balance(Rs):",
                   font=(None, 14), bg="#41525b")
        L2.place(x=82, y=120)
        c.execute("SELECT balance FROM Database WHERE accno=(?)", (accno,))
        balance = c.fetchone()
        L4 = Label(r1, text=balance, font=(
            None, 14), bg="#41525b", fg="#ff0000")
        L4.place(x=267, y=120)
        L3 = Label(r1, text="Enter amount to \nbe Withdrawn:",
                   font=(None, 13), bg="#41525b")
        L3.place(x=85, y=170)
        T3 = Entry(r1)
        ...
        B2 = Button(r1, text="Exit", command=r1.destroy,
                    padx=7, bg="darkred", fg="yellow")
        B2.place(x=410, y=0)
    ...
    def Deposit():
        ...
        def Check2():
            if T3.get() == "":
                ...
                conn.commit()
                messagebox.showinfo(
                    "Succesfull!", "Rs"+T3.get()+" is Deposited\nNew Balance: Rs"+str(bal[0]))
                T3.delete('0', END)
                L4['text'] = bal[0]
                r2.destroy()
            conn.commit()
        ...
        r2.title("Deposit")
        L1 = Label(r2, text="Deposit Menu", bg="darkblue",
                   fg="orange", font=(None, 25), padx=70)
        L1.place(x=60, y=50)
        L2 = Label(r2, text="Current Balance(Rs):",
                   font=(None, 14), bg="#41525b")
        L2.place(x=82, y=120)
        c.execute("SELECT balance FROM Database WHERE accno=(?)", (accno,))
        balance = c.fetchone()
        L4 = Label(r2, text=balance, font=(
            None, 14), bg="#41525b", fg="#ff0000")
        L4.place(x=267, y=120)
        ...
        B1.place(x=170, y=230)
        B2 = Button(r2, text="Exit", command=r2.destroy,
                    padx=7, bg="darkred", fg="yellow")
        B2.place(x=410, y=0)
    ...
    if flag == True:
        ...
        r.title("Main Menu")
        B1 = Button(r, text="Withdraw", bg="#0b0", font=(
            None, 15), padx=2, command=Withdraw)
        B1.place(x=30, y=30)
        B2 = Button(r, text="Deposit", bg="#0b0", font=(
            None, 15), padx=7, command=Deposit)
        B2.place(x=150, y=30)
        B4 = Button(r, text="Exit", command=r.destroy,
                    padx=7, bg="darkred", fg="yellow")
        B4.place(x=260, y=0)
...
def Generate():
    ...
    if (c.fetchall()):
        ...
        client.messages.create(to="8431866878", from_="+17179298914", body=otp)
        messagebox.showinfo(
            "Disclaimer", "A 4-digit OTP has been sent to your Registered no.")
...
L3 = Label(root, text="Enter Credentials", font=(
    None, 25), bg="navyblue", fg="white", padx=80)
L3.place(x=120, y=25)
...
B2 = Button(root, text="  Submit  ", command=Submit,
            bg="#6f6", state="disable")
